extends Node3D
## Standalone building screenshot tool — renders a building prefab from several angles and saves PNGs,
## so the look can be iterated (and details across angles not missed) without a playtest. Needs a real
## renderer (a GPU + display, or xvfb) — NOT --headless.
##
## Run: godot --path . client/art/preview/building_preview.tscn -- --building=<name> --shot=<prefix>
## Saves <prefix>_iso.png, <prefix>_front.png, <prefix>_side.png, <prefix>_eye.png
...
func _ready() -> void:
	...
	print("[preview] saved %s_{iso,front,side,eye}.png building=%s" % [prefix, bname])
	get_tree().quit()
...
func _aim(bb: AABB, view: String) -> void:
	...
	match view:
		...
		"eye":  # low, close-in 3/4 — catches door/window height + interior through openings
