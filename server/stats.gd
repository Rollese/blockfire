...
# LMG nests (M19 P4)
var nests_deployed := 0   # M19 P4: LMG nests deployed (cumulative)
var nests_manned := 0     # M19 P4: nest mounts (cumulative)
var nests_destroyed := 0  # M19 P4: nests destroyed (cumulative)
# Grapple hook (M19 Assault) — cumulative, NOT window-reset (mirrors nests/shield)
var grapples_deployed := 0   # M19: grapple ladders deployed (hook fired + resolved)
var grapple_climbs := 0      # M19: climb-starts on a DEPLOYED (grapple) ladder
var grapple_cuts := 0        # M19: deployed grapple ladders cut down
# Magazine ammo (M2) — cumulative, NOT window-reset
var mags_dropped := 0        # M2 ammo: mags dropped via hold-R fast reload
var mags_picked := 0         # M2 ammo: dropped mags reclaimed via PICKUP_MAG
var redistributes := 0       # M2 ammo: partial-mag consolidations performed
