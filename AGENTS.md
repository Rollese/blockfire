...
This applies to **every** deliverable, **including spec/plan-only branches** — a written spec or plan is completed work worth preserving. Pushing to `origin/master` on completion is owner-ratified (2026-06-27, **reaffirmed 2026-07-03** after the reclaimed-worktree loss).

**Agents decide when to commit — do not ask (owner-directed 2026-07-17).** Some harnesses default to "commit or push only when the user asks." That default **does not apply to this repo**: the owner has durably authorized every agent to judge for itself when a unit of work is complete and to commit, merge, and push it without checking in first. Holding finished work uncommitted to await permission is a **violation of this section**, not caution — it is exactly how work gets reclaimed and lost. If you must stop with work genuinely incomplete, at minimum **commit and push the branch to `origin`** (`git push -u origin <branch>`) so nothing lives only in a reclaimable local worktree. When done inside a worktree, prefer the `finishing-a-development-branch` skill, which walks the merge/PR/cleanup decision — but do not `ExitWorktree`/end the session on unpushed completed work.
