...
*Throwables block (Checkpoint 3):* `SELF_STATE` is extended with a trailing **variable-length** `throwables` list `[{kind, count}]` so the HUD selector shows live counts. It is **append-only and back-compatible** — decode reads the block only if bytes remain, defaulting to `[]` for pre-C3 (4-field) senders. The list is **data-driven** (frag/smoke share the grenade cooldown → `count = 1` when ready else `0`; RPG appends `{kind:100, count:rockets}`), so M5.5 throwables (flashbang/impact) slot in by adding list entries with no UI/wire rewrite. `kind:100` is a UI-only RPG tag pending M5.5 formalization.
...
- A new pure `DeploySpawn` helper (`shared/sim/`) **enumerates** valid refs and **resolves/validates** a ref → position. The ref scheme: `0` = HQ; `1..N` = owned capture point; `SQUADMATE_BASE(200)+i` = squadmate at index `i`; `VEHICLE_BASE(220)+i` = friendly vehicle at index `i` (squadmate/vehicle refs landed in Checkpoint 3). Validity rules live **in `DeploySpawn`** (shared): a squadmate ref is valid only if the mate is alive + standing + same-team; a vehicle ref only if same-team + has a free seat. The server and client each pass their own candidate arrays (`squadmates`/`vehicles`) built from the same shapes (`{pos,team,alive,downed}` / `{pos,team,free_seats}`), so the same rule applies on both sides. The client lists `DeploySpawn.enumerate(...)`; the server **re-validates** the requested ref via `DeploySpawn.is_valid(...)` against its **authoritative** candidate arrays and places the pawn, or ignores it (mate died / vehicle filled / point lost) and the client re-prompts. **No spawn-placement logic moves to the client** — it only sends intent over choices the server re-validates.
...
Sent reliably to the **victim** on true death: `{killer_id, weapon, distance, killer_hp, attackers:[{id, dmg}]}`. The server keeps a **per-life damage ledger** (`attacker_id → applied dmg`, accrued in `_apply_pawn_damage` for tracked clients only, skipping no-attacker/world damage id 0) and clears it at the start of each life (deploy + auto-respawn). On death the ledger is ordered by the pure `DeathRecap.attackers_sorted` (damage desc, id asc) into the `attackers` list. **Presentation only** — it does not affect authoritative state; names are resolved client-side from `ROSTER`. The payload rides the server-confirmed model (projectile-compatible — no hit-scan assumption), so M5.5 weapons need no recap rewrite. No position reveal, no killcam.
