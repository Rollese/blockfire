...
**Bot RPG-at-structure aggression — TEMPERED 2026-07-09 (was a 2026-07-06 balance gap).** Engineer bots used to fire RPGs freely: with vehicles removed from every shipping map, the anti-vehicle `maybe_rpg` *always* fell through to `maybe_rpg_building`, which rocketed the nearest wall every `RPG_FIRE_COOLDOWN` (~4 s) for the whole match. An owner playtest measured ~**815 rockets in 28 min** across ~24 bots → ~**27 %** of `conquest_town`'s pieces destroyed and 12 collapses — a player joining a mature match dropped into a heavily-wrecked map (**high, un-BattleBit-like**, though not a correctness bug). **Fix:** a structure-specific cadence `RPG_STRUCTURE_COOLDOWN` = **600 ticks (~20 s, 5× the anti-vehicle path)** plus an **enemy-cover target preference** — `BotExercisers.rpg_structure_target` prefers the nearest piece a visible enemy is using as cover, falling back to the nearest piece only so the destruction mechanic still exercises on sparse maps. Measured **~7× fewer rockets** (M11 GATE-A on `conquest_town`: `rockets=7` in 103 s ≈ 114/28 min vs. 815). **Validation note:** run the M11 destruction gate on **`conquest_town`** (buildings sit at the capture points) for bot-combat/RPG changes — `conquest_proving_grounds` has 4 bunkers far from combat and its destruction signal sits at the noise floor (control: `rockets=1 destroyed=1`, one roll from failing even pre-change), so it is not a reliable regression gate for RPG-*rate* tuning. Evidence: `docs/gate-evidence/20260709-bot-rpg-restraint.txt`; decision helper unit-tested in `tests/bot_rpg_restraint_test.gd`. `--fast-rpg`/`--fast-nades` remain **humans-only**.
