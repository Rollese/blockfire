...
4. **Audio assets.** This branch ships the **catalog + engine only**; logical `stream` ids resolve to real `.ogg`/`.wav` (or generated placeholder tones) at wiring time. Asset source **resolved 2026-07-03 (§12.6)**: CC0/royalty-free packs curated by the owner, with procedural synthesis as the variant/placeholder fallback.
