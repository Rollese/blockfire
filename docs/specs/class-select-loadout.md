...
| Gadget | **Two per class, always one selected** | Removes the empty-slot edge case; gives each class a clear pick. |
...
| Claymore | **Removed entirely** | Owner call — trims the gadget list. |
...
### Per-class
| Class | Primary options | Gadget (pick one) | Passive identity |
|---|---|---|---|
| **Assault** | AR · SMG · DMR | **C4** · **Sandbag**\* | DMR access · **Combat Vigor** (faster health regen) |
| **Medic** | AR · SMG | **Medkit** · **Combat Stim** (3 charges, refillable) | faster revive · 20 bandages |
| **Engineer** | AR · SMG | **RPG** · **C4** | sledgehammer · **+20% explosive blast radius** |
| **Support** | AR · SMG · **LMG** | **Ammo Bag** · **Sandbag**\* | +reserve ammo · LMG suppression · **5 grenades** |
...
\* **Sandbag** is the P1 interim second gadget for Assault & Support — a deployable cover piece reusing the M4/M12 build/structure system. The **follow-up milestone** swaps it for **Grappling Hook** (Assault) and **Riot Shield** (Support), which need new mechanics (movement / carried collision) out of scope here.
...
- `gadget_options(cls) -> Array[int]`: Assault `[GADGET_C4, GADGET_SANDBAG]`, Medic `[GADGET_MEDKIT, GADGET_STIM]`, Engineer `[GADGET_RPG, GADGET_C4]`, Support `[GADGET_AMMO, GADGET_SANDBAG]`.
...
- New gadget id constants: `GADGET_RPG`, `GADGET_MEDKIT` (rename of HEAL), `GADGET_STIM`, `GADGET_SANDBAG` alongside existing `GADGET_C4`, `GADGET_AMMO`. `GADGET_MINE` (claymore) is **removed** from the option sets (the mine sim stays in the tree but is no longer selectable — see §J).
...
## D. RPG as a gadget (`shared/sim/` gadget system, `server/server_main.gd`)

RPG moves from **weapon-primary slot** to the **gadget system**:
- `GADGET_RPG` is an Engineer gadget option. When selected, the player's launcher is equippable via the gadget/weapon-wheel path (like other deployables), firing rockets with the existing rocket pool + cooldown (`rockets`, `last_rocket_tick`, `KIND_RPG` ammo).
- The primary weapon slot for a RPG-Engineer is now a real gun (AR/SMG) — the "broken weapon" human-exclusion (`random_class_no_engineer`) is **deleted**; humans pick Engineer freely.
- Server spawn: rocket pool is seeded when `gadget == GADGET_RPG` (not when `weapon == RPG`). The `Weapon.RPG` enum entry may stay for the projectile/def, but it is never a `primary` in a sanitized human/bot loadout.

> **Implementation note:** this is the one genuine sim rework in M18. Keep the rocket firing/projectile path identical; only the *ownership/slot* changes (gadget selection seeds the launcher instead of the primary weapon id). Covered by the P1 gate + unit tests.
...
- `loadout_options`: `primary_options`/`gadget_options` per class match the matrix; `default_loadout` is self-consistent (passes sanitize unchanged).
...
- **P1:** 128-bot match on `conquest_town` exercising every class × every armor tier × each gadget (incl. RPG gadget + LMG), tick mean **< 33.3 ms** at 30 Hz, bandwidth unchanged (no per-tick additions), Conquest reaches a winner. Fleet gate on game2.
- **P2:** trait correctness in-sim (blast radius, regen, stim buff, grenade counts) via the deterministic drill.
- **P3:** client screen — screenshot QA + owner playtest (visual FEEL deferred to owner, project norm).
...
- **P1 — Sim + wire framework.** `LoadoutConfig` + `sanitize` + option/trait tables; LMG weapon; RPG-as-gadget; armor/grenade/gadget as player-picked; `SET_LOADOUT` (VERSION 8); per-connection persistence; server applies on spawn; bots on the unified path (incl. Sandbag interim gadget). Deterministic tests + 128-bot fleet gate. **No client screen yet** (auto/`--loadout=` debug arg drives it headless).
- **P2 — Class traits & new gadgets.** Combat Vigor, +20% blast, Combat Stim (refillable), Support grenades/reserve, LMG suppression — wired to `class_traits` and gate-proven via the drill.
- **P3 — Client class-select screen.** The UI + screenshot QA + owner playtest.

*(P1 and P2 may merge if the trait wiring is small; the gate boundary is what matters.)*

## M. Out of scope (explicit)

- **Grappling Hook (Assault) & Riot Shield (Support)** — the "real" second gadgets; need new movement/collision mechanics. Sandbag is the interim. Follow-up milestone.
...
- **Binoculars / spotting system** — not built; the medic's team utility is the Combat Stim.
- **Claymore/mine as a selectable gadget** — removed from the option sets (sim code retained but unreachable via loadout).
