...
- **M4 Building & destruction** — **Phase 1 (Building) IN PROGRESS.** Spec `docs/specs/building.md`, plan `docs/plans/2026-06-15-m4-building.md` (15 TDD tasks) done on branch `m4-building`; 124 unit tests green. **Laptop-48 gate PASS 2026-06-15** (`ci/m4_building_test.sh BOTS=48`: winner declared, peak tick 18.3ms < 33.3, struct/builds/blocked_shots all > 0, structures synced). **Fleet-128 gate pending** (run on the unraid Docker fleet — see `docs/milestones/M4-building-destruction.md`). Then merge `m4-building` → master. Phase 2 (Destruction, `docs/specs/destruction.md`) is the next milestone phase.
...
## Next
- **M4 Phase 1 (Building)** — laptop-48 gate PASS; **run the 128-bot fleet gate** on the unraid W-2275 (`cd docker && SERVER_CPUS=0,1,14,15 BOTS_CPUS=2-13,16-27 ./run-gate.sh`; point `run-gate.sh` at `ci/m4_building_test.sh` for the M4 `struct/bld/blk` assertions). Record the result in `docs/milestones/M4-building-destruction.md`, then merge `m4-building` → master and `git push origin master`. If the fleet over-blocks (no winner / tickets stall), lower bot build density — `MAX_BOT_BUILDS` in `bots/bot_driver.gd`. Watch the tick budget: building adds per-tick cost on top of the tuned `_send_snapshots`; profile (`[perf]` telemetry) on the fleet.
- **M4 Phase 2 (Destruction)** — `docs/specs/destruction.md` (sketched in `building.md` §K). Start with `brainstorming` → spec, per the working agreement.
