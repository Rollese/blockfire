...
## Next
- **M4.5 Combat Depth & class identity** — M4 is closed. Start with `brainstorming` → `docs/specs/combat-depth.md` (milestone: `docs/milestones/M4.5-combat-depth.md`): DBNO/revive/bandages, body dragging, ladders, vaulting, drop-shoot prevention, class gadgets (C4/mines/ammo bags/RPG/medic bag), weapon attachments, bullet penetration. Its own 128-bot gate (tick + bw budget, Conquest winner). **Reuse what M4-P2 already built:** the `Grenade` ballistic model + frag/smoke detonation path and the `GRENADE_THROW(type)`/`SMOKE_DEPLOYED` protocol — the RPG is a frag-like projectile, and the Assault class wires frag/smoke into gadget slots. **Watch the tick budget:** the 128-bot fleet peak rides the edge (~29.5 ms in clean runs, one 35.39 ms contention spike) and `snap` (~16 ms) is the dominant pre-existing cost — profile `[perf]` on the fleet early and lean on the degradation knobs (`MAX_STRUCTURE_DELTAS_PER_TICK`, gadget caps) before adding per-tick work.
- **Fleet gate how-to:** `docker/run-gate.sh` (M3 assertions) on the unraid W-2275 (`cd /mnt/app/blockfire/docker && SERVER_CPUS=0,1,14,15 BOTS_CPUS=2-13,16-27 docker compose --profile full up -d --build`); `docker/run-m4-gate.sh` adds the M4 assertions (P1: `struct/bld/blk` + `structures synced`; P2: `destroyed>=1`, `nades>=1`, with `splash/smoke` reported). Stay confined to `/mnt/app/blockfire` on that host (AGENTS.md §8). If a fleet match over-blocks, lower `MAX_BOT_BUILDS`/`MAX_BOT_GRENADES` in `bots/bot_driver.gd`.
