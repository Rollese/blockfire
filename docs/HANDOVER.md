...
- **M4 Building & destruction** — **NEXT** (M3 fully passed). Start with `brainstorming` → spec, per the working agreement.
...
## Next
- **M4 (Building & destruction)** — M3 is fully closed (gate PASS at 128 on the fleet). Start with `brainstorming` → spec, per the working agreement. Watch the tick budget: building/destruction adds per-tick cost on top of the now-tuned `_send_snapshots`; profile (`[perf]` telemetry) on the fleet early.
