...
### 2. Squad-leader FOB replaces free squad spawning as the primary forward spawn

- The squad's forward spawn becomes a **squad-leader-built FOB (Forward Operating Base)**: one active FOB per squad, server-authoritative placement.
- Squad members spawn at the FOB **unless an enemy is within its vicinity radius**, in which case FOB spawning is temporarily **disabled** while the **FOB itself persists** (no proximity-destroy).
...
*Why:* M3 ratified "spawn on any alive squadmate," which lets a whole squad teleport to any pushed member with no counterplay. Routing forward spawning through a leader-placed, enemy-suppressible FOB gives the enemy a way to shut down a spawn flood by pressuring its location, rewards good FOB placement, and gives the squad leader a meaningful tactical role — closer to Battlefield/Squad's FOB/rally model. This **supersedes the M3 squad-spawn decision** (M3 doc carries a superseded note).
...
- **M12 (squad FOB & class refit)** is scheduled **near-term, before/alongside the in-progress M7 client**, so M7 builds the 4-class select + FOB deploy/spawn UI rather than building Recon / squadmate-only-spawn UI it would later rip out. Spec: [`squad-fob-class-refit`](../specs/squad-fob-class-refit.md).
...
- **M3** and **M4.5** milestone docs gain superseded-decision notes pointing here (squad spawn → FOB; Recon class removed / claymore reassigned).
