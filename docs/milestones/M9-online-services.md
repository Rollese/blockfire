...
## Gate
A player authenticates via Steam, is placed into a rating tier, and is matched (with dynamic tier-merge under low population) into an official 128-slot server; match results flow back as signed reports and update the rating; a seeded cheat-like input trace is flagged by Layer 4 with measured false-positive rate on legitimate traces; forged/community match reports are rejected.
