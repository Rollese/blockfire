...
| M12 | [Squad FOB & class refit (Recon removal)](milestones/M12-squad-fob-class-refit.md) | todo | **Sequence: near-term — before/with the M7 client.** Remove Recon (4 classes); DMR Assault-only, no sniper rifles; claymore → Engineer (pick-one vs C4); squad-leader **FOB** replaces free squad spawn (squadmate-rally kept as fallback), spawning disabled while an enemy is in the FOB's vicinity. Gate: 128-bot Conquest with FOBs placed/used/enemy-disabled, no Recon, DMR Assault-only, budget held, winner declared. Decision: [ADR-0007](adr/0007-battlebit-divergences.md). |
