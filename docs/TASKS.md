...
| M12 | [Squad FOB, cooperative construction & class refit](milestones/M12-squad-fob-class-refit.md) | **COMPLETE ✅ (P1+P2+P3)** | **P3 — squad-leader FOB CLOSED 2026-06-29** (128-bot fleet gate PASS on game2: `winner=0 elapsed=256s peak tick=22.48ms<33.3 fobs_built=1 fobs_destroyed=1` — a squad FOB cooperatively shovelled to completion live, then destroyed in-match; across two runs built/disabled/destroyed all fired emergently; mechanics proven deterministically in `fob_functional_test`; `docker/srvlog-20260629-235035.log`): leader-only `PLACE_FOB`→FOB build site→M4-destructible bunker spawn (completed+not-destroyed+no-enemy-within-40m), `_reconcile_fobs` lifecycle, `FOB_LIST`/`FOB_BASE` replication for M7; squadmate-rally fallback kept; client FOB UI/ghost/bunker model deferred to M7. **P2** — cooperative shovel construction CLOSED 2026-06-28 (`built_large=1`; `docker/srvlog-20260628-225159.log`). **P1** — class refit CLOSED 2026-06-28 (`c4=15 mines=3`; `docker/srvlog-20260628-184043.log`). | **Sequence: near-term — before/with the M7 client.** Phased: **P1 — class refit CLOSED 2026-06-28** (128-bot fleet gate PASS on game2: `winner=1 elapsed=345s peak tick=28.47ms<33.3`, **C4 `c4=15` + claymore `mines=3` both fire** — Engineer Gadget-A split proven at scale; no Recon, DMR Assault-only unit-proven; `docker/srvlog-20260628-184043.log`): remove Recon (4 classes), DMR Assault-only (no sniper rifles), claymore→Engineer (pick-one vs C4) — *independent of M11, lands first*. **P2** universal **shovel** construction — placing a piece creates a build site shovelled to completion; small=solo, large/FOB=≥2 builders (supersedes M4 instant placement; reuses M4 destruction). **P3** squad-leader **FOB**: a cooperatively-built, **destructible** bunker spawn — proximity-disabled while enemies near, destroyed via M4 destruction, hunted by enemy AI; squadmate-rally kept as fallback. P2/P3 sequenced after M11 (shared structure store/protocol). Decision: [ADR-0007](adr/0007-battlebit-divergences.md). |
...
| Hand ballistics model to **M7-C3** | — | todo | C2 done; M7-C3 (combat-depth UI) must build projectile-aware prediction (cosmetic tracer; server-confirmed hits), not hit-scan. See spec §1 + M7 ⚡ callout. The shared integrator now lives in `shared/sim/projectile.gd` (ready for the client tracer). |
