...
- **Server (`server_main.gd` + support seam)**:
  ...
  - **Disconnect cleanup (extend `_on_peer_disconnected`, `server_main.gd:2738`)** — drop the leaver's deployed ladders by owner id, exactly like the existing C4/mine cleanup there. **Companion fix (owner-requested):** the same handler must also drop the leaver's **LMG nest / emplacements** — currently a gap: `_emplacements` is keyed by the same `owner_id` and already enforces one-nest-per-owner in `deploy()` (`emplacement_server.gd:31-35`), but nothing removes a leaver's nest on disconnect, so it lingers for the match while their C4 and mines are already cleaned. Add a `ServerEmplacement.remove_owner(id)` helper (mirroring the deploy-time owner-eviction loop) and call it from the disconnect handler. Result: all owner-deployables (ladder, nest, C4, mine) share one consistent disconnect sweep, and both nest and ladder already share the same one-per-owner-across-lives deploy limit.
