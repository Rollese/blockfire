...
func test_fob_is_a_wide_low_bunker() -> void:
	...
	var fob := StructureKit.aabb(StructureKit.build("fob", 3))
	var wall := StructureKit.aabb(StructureKit.build("wall", 3))
...
func test_heavy_barricade_is_thicker_than_a_wall() -> void:
	var bar := StructureKit.aabb(StructureKit.build("heavy_barricade", 3))
	var wall := StructureKit.aabb(StructureKit.build("wall", 3))
