...
func test_hull_is_team_tinted() -> void:
	var hull := autofree(VehicleKit.build("transport", 1)).get_node("Hull") as MeshInstance3D
...
func test_unknown_vehicle_falls_back_to_transport() -> void:
	assert_true(autofree(VehicleKit.build("mystery", 0)).has_node("Hull"), "unknown name renders as transport")
