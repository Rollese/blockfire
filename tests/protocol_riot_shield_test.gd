...
func test_version_is_13() -> void:
	assert_eq(Protocol.VERSION, 13, "VERSION bumped for the shield wire")
...
func test_self_state_shield_frac_absent_defaults_to_zero() -> void:
	...
	var full := Protocol.encode_self_state(30, false, 0, 0, [], false, 0.0, 0, 0, false, 0.0, 0.0,
		100.0, 0.0, true, false, 0, 0.0, false, 0, false, 0, 0, 0, 0,
		0, 0, 0, false, 255)
	var old := full.slice(0, full.size() - 3)   # strip the M2 spare_mags-count byte + the M19 grapple byte + the 1-byte shield tail
