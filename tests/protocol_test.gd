...
func test_self_state_vault_defaults_when_absent() -> void:
	...
	b.resize(b.size() - 21)   # drop the two vault bytes + trailing regen + sprint-locked + input-buf-depth + M16 bleeding/progress + M17 reserve + M19 stim + M19 P4 mount-tail + M19 P5 shield-frac bytes
	var d := Protocol.decode_self_state(b)
...
func test_self_state_regen_defaults_when_absent() -> void:
	...
	b.resize(b.size() - 19)   # drop the M19 stim + M17 reserve + M16 bleeding/progress + input-buf-depth + sprint-locked + regen + M19 P4 mount-tail + M19 P5 shield-frac bytes
	var d := Protocol.decode_self_state(b)
...
func test_self_state_sprint_locked_defaults_when_absent() -> void:
	var b := Protocol.encode_self_state(17, false, 0, Weapon.AR, [], false, 0.0, 0, 0, false, 0.0, 0.0, 100.0, 0.0, true, false, 0, 0.0, true)
	b.resize(b.size() - 18)   # drop the M19 stim + M17 reserve + M16 bleeding/progress + input-buf-depth + sprint-locked + M19 P4 mount-tail + M19 P5 shield-frac bytes
	var d := Protocol.decode_self_state(b)
...
func test_self_state_input_buf_depth_absent_is_sentinel() -> void:
	...
	var b := Protocol.encode_self_state(17, false, 0, Weapon.AR, [], false, 0.0, 0, 0, false, 0.0, 0.0, 100.0, 0.0, true, false, 0, 0.0, false, 3)
	b.resize(b.size() - 17)   # drop the M19 stim + M17 reserve + M16 bleeding/progress bytes + the input-buf-depth byte + M19 P4 mount-tail + M19 P5 shield-frac bytes
	var d := Protocol.decode_self_state(b)
