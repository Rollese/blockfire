...
func test_place_chunk_remove_mirror() -> void:
	var s := {}
	VC.apply_structure_delta(s, {"op": Protocol.OP_PLACE, "rec": {"id": 5, "chunks": ChunkMask.full_mask(8)}})
	assert_eq(s.has(5), true)
	var newmask := ChunkMask.full_mask(8) & ~0b111
	VC.apply_structure_delta(s, {"op": Protocol.OP_CHUNK, "id": 5, "mask": newmask})
...
func test_chunk_on_unknown_id_is_noop() -> void:
	var s := {}
	VC.apply_structure_delta(s, {"op": Protocol.OP_CHUNK, "id": 9, "mask": 0})
