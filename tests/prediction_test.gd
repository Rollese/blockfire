...
func test_replay_converges_after_authoritative_correction() -> void:
	...
	var auth := Vector3(Stance.speed(Stance.STAND) * Armor.speed_mult(Armor.LIGHT) * SimLoop.DT - 0.5, 0, 0)
	...
	var expected := auth.x + 2.0 * Stance.speed(Stance.STAND) * Armor.speed_mult(Armor.LIGHT) * SimLoop.DT
