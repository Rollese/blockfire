...
func test_nearest_free_own_vehicle_picks_closest_with_a_free_seat() -> void:
	...
	var got := VC.nearest_free_vehicle(vview, Vector3.ZERO)
...
func test_drive_dir_points_at_objective() -> void:
	...
	var cmd := VC.drive_toward(0.0, Vector3.ZERO, Vector3(50, 0, 0))
...
func test_drive_toward_steers_right_to_face_target() -> void:
	...
	var c := VC.drive_toward(0.0, Vector3.ZERO, Vector3(50, 0, 0))
...
func test_drive_toward_no_steer_when_aligned() -> void:
	...
	var c := VC.drive_toward(0.0, Vector3.ZERO, Vector3(0, 0, 50))
...
func test_vehicle_is_enemy_true_when_enemy_occupant() -> void:
	...
	var view := {42: enemy}
	assert_true(VC.vehicle_is_enemy(v, view, 0))   # my_team 0, occupant team 1 -> enemy

func test_vehicle_is_enemy_false_when_friendly_or_empty() -> void:
	var v := VehicleState.new(); v.seats = [42, 0, 0, 0, 0]
	var friendly := EntityState.new(); friendly.team = 0
	assert_false(VC.vehicle_is_enemy(v, {42: friendly}, 0))   # same team
	var empty := VehicleState.new(); empty.seats = [0, 0, 0, 0, 0]
	assert_false(VC.vehicle_is_enemy(empty, {}, 0))           # empty
...
func test_nearest_enemy_vehicle_picks_closest_enemy_in_range() -> void:
	...
	var view := {7: e7, 8: e8}
	assert_eq(VC.nearest_enemy_vehicle(vview, view, Vector3.ZERO, 0, 60.0), Vehicle.id_for(0))
...
func test_nearest_enemy_vehicle_zero_when_out_of_range() -> void:
	...
	assert_eq(VC.nearest_enemy_vehicle({Vehicle.id_for(0): v}, {7: e}, Vector3.ZERO, 0, 60.0), 0)
...
func test_enemy_spawn_pos_returns_other_team_spawn() -> void:
	...
	assert_eq(VC.enemy_spawn_pos(spawns, 0, Vector3.ZERO), Vector3(900, 0, 0))
	assert_eq(VC.enemy_spawn_pos(spawns, 1, Vector3.ZERO), Vector3(-900, 0, 0))

func test_enemy_spawn_pos_falls_back_when_none() -> void:
	assert_eq(VC.enemy_spawn_pos([], 0, Vector3(5, 0, 5)), Vector3(5, 0, 5))
...
func test_nearest_enemy_pos_picks_closest_enemy() -> void:
	...
	var r := VC.nearest_enemy_pos(view, 99, 0, Vector3.ZERO)
...
func test_nearest_enemy_pos_none_when_only_friendlies_or_dead() -> void:
	...
	assert_false(bool(VC.nearest_enemy_pos({1: mate, 2: deadfoe}, 99, 0, Vector3.ZERO)["found"]))
